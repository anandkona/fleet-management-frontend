...
const REPAIR_SERVICES = [
  { name: 'General Repair', defaultPrice: 1000 },
  { name: 'Body Work', defaultPrice: 5000 },
  { name: 'Electrical Fix', defaultPrice: 1500 },
  { name: 'Engine Overhaul', defaultPrice: 15000 },
  { name: 'Tire Replacement', defaultPrice: 3500 },
  { name: 'Suspension Repair', defaultPrice: 2500 }
];

export default function MechanicRepairsPage() {
  ...
  const handleWorkflow = async (id, action) => {
    try {
      setProcessingId(id);
      await api.post(`/repairs/${id}/${action === 'close' ? 'complete' : action}`);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (!tempServiceType && REPAIR_SERVICES.length > 0) {
        setTempServiceType(REPAIR_SERVICES[0].name);
      }
  ...
  const handleAddLineItem = () => {
    const svc = REPAIR_SERVICES.find(s => s.name === tempServiceType);
  ...
  const handleSave = async () => {
    try {
      ...
      const payload = {
        ...
        issueDescription: form.description || 'Repair request',
        estimatedCost: calculatedCost > 0 ? calculatedCost : (editItem?.estimatedCost || 0),
        repairDate: form.date ? new Date(form.date).toISOString() : new Date().toISOString(),
        notes: form.notes || ''
      };

      const maintenanceId = editItem?.id || editItem?._id;
      if (editItem && maintenanceId) {
        await api.patch(`/repairs/${maintenanceId}`, payload);
        addNotification('Success', 'Repair record updated successfully', 'success');
      } else {
        await api.post('/repairs', payload);
        addNotification('Success', 'Repair scheduled successfully', 'success');
      }
      setOpenDialog(false); setEditItem(null); fetchData();
    } catch (err) {
      addNotification('Error', 'Failed to save repair record', 'error');
    }
  };
  ...
  const selectedService = REPAIR_SERVICES.find(s => s.name === tempServiceType);
    ...
    <Box sx={{ pt: 1, px: 3, pb: 3, maxWidth: '100%' }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <PageHeader 
          ...
          subtitle="Manage and update your repair schedules"
        />
        <Button 
          variant="contained" 
          onClick={() => { 
                  ...
                  <TableCell align="right" sx={{ whiteSpace: 'nowrap', borderBottom: '1px solid', borderColor: 'divider' }}>
                    <Stack direction="row" spacing={0.5} justifyContent="flex-end">
                      {(!t.status || t.status === 'DRAFT' || t.status === 'OPEN') && (
                        ...
                        {(t.status === 'OPEN' || t.status === 'IN_PROGRESS' || t.status === 'APPROVED') && (
                        ...
                        {(t.status === 'DRAFT' || t.status === 'OPEN' || t.status === 'APPROVED' || t.status === 'SCHEDULED' || t.status === 'IN_PROGRESS') && (
      ...
      <Dialog open={openDialog} onClose={() => setOpenDialog(false)} maxWidth="sm" fullWidth fullScreen={isMobile}>
        <DialogTitle sx={{ bgcolor: 'background.paper', color: 'text.primary' }}>Edit Repair</DialogTitle>
        <DialogContent sx={{ bgcolor: 'background.paper', pt: 2 }}>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 2 }}>
              ...
              <TextField label="Description *" value={form.description || ''} onChange={e => setForm({ ...form, description: e.target.value })} fullWidth size="small" placeholder="Describe the repair..." />
            </Box>
            <Box sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 1, p: 2 }}>
              <Typography variant="subtitle2" sx={{ mb: 1, fontWeight: 600 }}>Repair Line Items</Typography>
