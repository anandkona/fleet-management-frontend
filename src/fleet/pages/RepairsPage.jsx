    ...
    <Box>
                    ...
                    <Stack direction="row" spacing={0.5}>
                      {t.status === 'OPEN' ? (
                        <>
                          ...
                          <Tooltip title="Preview"><IconButton size="small" onClick={() => setPreviewItem(t)} sx={{ bgcolor: '#3b82f615', color: '#3b82f6', '&:hover': { bgcolor: '#3b82f630' } }}><VisibilityIcon sx={{ fontSize: 17 }}  /></IconButton></Tooltip>
                          {hasPermission('repair_update') && <Tooltip title="Cancel Repair"><IconButton disabled={processingId === (t.id || t._id)} size="small" onClick={() => handleWorkflow(t.id || t._id, 'cancel')} sx={{ bgcolor: '#ef444415', color: '#ef4444', '&:hover': { bgcolor: '#ef444430' } }}><CancelIcon sx={{ fontSize: 17 }}  /></IconButton></Tooltip>}
                        </>
                      ) : (
                        <>
                          {(!t.status || t.status === 'DRAFT') && hasPermission('repair_update') && <Tooltip title="Edit"><IconButton disabled={processingId === (t.id || t._id)} size="small" onClick={() => { setEditItem(t); setForm({ vehicleId: t.vehicleId || '', tripId: t.tripId || '', driverId: t.driverId || '', repairDate: t.repairDate ? t.repairDate.split('T')[0] : '', category: t.category || REPAIR_SERVICES[0]?.name || 'General Service', description: t.description || '', estimatedCost: t.estimatedCost || REPAIR_SERVICES.find(s => s.name === t.category)?.defaultPrice || '', actualCost: t.actualCost || '', provider: t.provider || '', invoiceNumber: t.invoiceNumber || '', status: t.status || 'DRAFT', notes: t.notes || '' }); setOpenDialog(true); }} sx={{ bgcolor: '#3b82f615', color: '#3b82f6', '&:hover': { bgcolor: '#3b82f630' } }}><EditIcon sx={{ fontSize: 17 }}  /></IconButton></Tooltip>}
                          {(t.status === 'CANCELLED' || t.status === 'COMPLETED') && <Tooltip title="Preview"><IconButton size="small" onClick={() => setPreviewItem(t)} sx={{ bgcolor: '#3b82f615', color: '#3b82f6', '&:hover': { bgcolor: '#3b82f630' } }}><VisibilityIcon sx={{ fontSize: 17 }}  /></IconButton></Tooltip>}
                          {(!t.status || t.status === 'DRAFT') && hasPermission('repair_update') && <Tooltip title="Start Repair"><IconButton disabled={processingId === (t.id || t._id)} size="small" onClick={() => handleWorkflow(t.id || t._id, 'start')} sx={{ bgcolor: '#64748b15', color: '#64748b', '&:hover': { bgcolor: '#64748b30' } }}><PlayArrowIcon sx={{ fontSize: 17 }}  /></IconButton></Tooltip>}
                          {t.status === 'IN_PROGRESS' && hasPermission('repair_update') && <Tooltip title="Complete Repair"><IconButton disabled={processingId === (t.id || t._id)} size="small" onClick={() => handleWorkflow(t.id || t._id, 'complete')} sx={{ bgcolor: '#10b98115', color: '#10b981', '&:hover': { bgcolor: '#10b98130' } }}><CheckCircleIcon sx={{ fontSize: 17 }}  /></IconButton></Tooltip>}
                          {(!t.status || t.status === 'DRAFT' || t.status === 'IN_PROGRESS') && hasPermission('repair_update') && <Tooltip title="Cancel Repair"><IconButton disabled={processingId === (t.id || t._id)} size="small" onClick={() => handleWorkflow(t.id || t._id, 'cancel')} sx={{ bgcolor: '#ef444415', color: '#ef4444', '&:hover': { bgcolor: '#ef444430' } }}><CancelIcon sx={{ fontSize: 17 }}  /></IconButton></Tooltip>}
                        </>
                      )}
                    </Stack>
                  </TableCell>
      ...
      >
        <DialogTitle sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid', borderColor: 'divider' }}>
          ...
          <IconButton onClick={() => setOpenDialog(false)} size="small" sx={{ bgcolor: '#ef444415', color: '#ef4444', '&:hover': { bgcolor: '#ef444430' } }}><CloseIcon sx={{ fontSize: 17 }}  /></IconButton>
        </DialogTitle>
